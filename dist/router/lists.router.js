...
import { OptionalAuth, RequiresAuth } from "../guard/auth.js";
...
router.get("/:list_id", OptionalAuth, async (req, res, next) => {
    try {
...
router.post("/", RequiresAuth, validateBody, async (req, res, next) => {
    try {
...
router.post("/:list_id/share", RequiresAuth, validateBody, async (req, res, next) => {
    try {
        const listId = Number(req.params.list_id);
...
router.patch("/:list_id", RequiresAuth, validateBody, async (req, res, next) => {
    try {
...
router.delete("/:list_id", RequiresAuth, async (req, res, next) => {
    try {
...
router.delete("/:list_id/share/:shared_user_email", RequiresAuth, async (req, res, next) => {
    try {
        const listId = parseInt(req.params.list_id);
        const sharedUserEmail = req.params.shared_user_email;
        if (!Number.isInteger(listId) || listId <= 0) {
            return next(new httpError(404, "Invalid list Id"));
        }
        const list = await prismaDB.todoList.findUnique({
...
router.delete("/:list_id/share", RequiresAuth, async (req, res, next) => {
    try {
        const user = await prismaDB.user.findUnique({
            where: { email: req.user.email },
            select: { id: true }
