...
router.get("/:list_id/items", OptionalAuth, async (req, res, next) => {
    try {
...
router.post("/:list_id/item", RequiresAuth, validateBody, async (req, res, next) => {
    try {
        const listId = Number(req.params.list_id);
...
router.get("/:list_id/item/:itemId", OptionalAuth, async (req, res, next) => {
    const listId = Number(req.params.list_id);
...
router.patch("/:list_id/item/:itemId", RequiresAuth, validateBody, async (req, res, next) => {
    try {
        const listId = Number(req.params.list_id);
...
router.delete("/:list_id/item/:itemId", RequiresAuth, async (req, res, next) => {
    try {
        const user = await prismaDB.user.findFirst({
            where: {
                email: req.user.email
